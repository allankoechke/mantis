...
namespace mantis
{
    ...
    bool Context::hasKey(const std::string& key) const
    {
        return data.contains(key);
    }

    DukValue Context::get(const std::string& key)
    {
    ...
    DukValue Context::getOr(const std::string& key, DukValue default_value)
    {
        auto val = get(key);
        if (val == DukValue{})
        {
    ...
    void Context::set(const std::string& key, const DukValue& value)
    {
        const auto ctx = MantisApp::instance().ctx();
